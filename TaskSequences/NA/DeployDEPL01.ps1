...
foreach($Role in $Roles){
    switch ($Role)
    {
        'DEPL' {
            ...
            Invoke-Command -VMName $($ServerData.ComputerName) -FilePath C:\Setup\Scripts\Set-FARoles.ps1 -ArgumentList $Role,$DataDiskLabel,$RunAsAccount,$RunAsAccountPassword -ErrorAction Stop -Credential $domainCred
        }
        Default {}
    }
}
