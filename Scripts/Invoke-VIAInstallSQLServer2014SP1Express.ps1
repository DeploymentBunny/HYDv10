...
switch ($SecurityMode)
{
    ...
    'Mixed' {
        ...
        add-Content $unattendFile "BROWSERSVCSTARTUPTYPE=""Automatic"""
        add-Content $unattendFile "IACCEPTSQLSERVERLICENSETERMS=""True"""
        Add-Content $unattendFile "SAPWD=P@ssw0rd"
        Add-Content $unattendFile "SECURITYMODE=SQL"
    }
    Default {}
}
...
try
    {
      ...
      $result = Start-Process -FilePath $SQLSetup -ArgumentList "/ConfigurationFile=$unattendFile"  -Wait -NoNewWindow -PassThru
      return $result
    }
Catch
    {
